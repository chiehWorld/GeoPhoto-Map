...
## Release v1.0 特性
- **应用名称更新**：将侧边栏标题从 "GeoPhoto" 更新为 "GeoPhoto Map"。
- **“NEEDS LOCATION” 默认折叠**：
  - 在侧边栏中为该区域添加了折叠/展开功能。
  - 默认状态设为折叠，点击标题行即可展开查看未定位的照片。
  - 添加了箭头图标（Chevron）来指示当前的折叠状态。
- **移除 “SOURCE DIRECTORIES”**：由于在 Docker 环境下目录映射是固定的，已从 UI 底部移除了该展示区域，使界面更加简洁。
